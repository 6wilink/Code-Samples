...
function user.conf.session()
  local f = conf.uci.get('wui', 'wui', 'id')
  if (f and f ~= '') then
    return f
  end
...
function user.conf.root()
	local ip = conf.uci.get('wui', 'wui', 'root')
  if (ip and ip ~= '') then
    return ip
  end
	return ''
...
function user.conf.user()
	local _user = conf.uci.get('wui', 'wui', 'user')
...
function user.conf.passwd()
	local _passwd = conf.uci.get('wui', 'wui', 'pass')
